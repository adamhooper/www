...
def main():
    ...
    cursor = db.cursor()
    for id, title, subtitle, published_at, url, image_html, hack_blurb, hacker_blurb in cursor.execute("SELECT id, title, subtitle, published_at, url, image_html, hack_blurb, hacker_blurb FROM portfolio_pieces"):
        ...
        content = '\n'.join([
            ...
            'subtitle: ' + yaml_quote_string(subtitle),
            'slug: ' + slug,
            'date: ' + published_at,
            'published_url: ' + yaml_quote_string(url),
            'image_html: ' + yaml_quote_string(image_html),
            '---',
            '##### For hacks',
            '\n',
            hack_blurb,
