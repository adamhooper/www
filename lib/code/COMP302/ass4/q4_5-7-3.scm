...
;
; I'm not 100% sure this is what the question asks for... but I figure the
; purpose is to demonstrate a basic understanding of how this all works. I do
; -- how else would I have actually gotten objects working in the first place!
;
...
; It also adds Chapter 5's "move work to class decl" object-oriented design
...
(define find-method-and-apply
  (lambda (m-name host-name self args)
    (if (eqv? host-name 'object)
      ...
      (let ((m (lookup-method m-name (class-name->methods host-name))))
	(if (method? m)
	  (apply-method m host-name self args)
	  (find-method-and-apply
