...
public class Main
{
    ...
    // {{{ writeSchool()
    /**
     * Outputs our school to a text file.
     */
    private static void writeSchool() throws IOException
    {
        People person;
        Student student;
        Prof prof;
        ...
        for (int i = 0; i < school_length; i++) {
            ...
            pw.print(person.getLastName() + " " + person.getFirstName() + " " +
                     person.getId() + " " + person.getType() + " ");

            if (person.getType() == 'S') {
                student = (Student) person;

                pw.print(student.getGpa() + " " +
                         (student.getFullTime() ? "T" : "S"));
            } else if (person.getType() == 'P') {
                prof = (Prof) person;

                pw.print(prof.getRating() + " " + prof.getSalary());
            }
